...
- VS4: Dialogue Replacement And Mix. `output/audio.wav`, `output/translation.json`, and `output/tts/*.wav` produce `output/mixed_audio.wav` and `output/mix_plan.json`. The implementation ducks the original source audio and overlays generated Khmer speech at approximate source segment start times with a configurable mix-layer `TTS_SYNC_OFFSET_MS` delay adjustment, defaulting to 200 ms.
...
- VS4 is covered by mix path naming, translation timing parsing, configurable TTS sync delay calculation, missing TTS skipping, FFmpeg mix command construction, mix-plan writing, mixed-audio output behavior, and no-TTS failure handling.
